...
-- Initialize export from 'nt_lib' resource
local export = exports[nt_lib]
...
local loadModule = function(self, module)
    ...
    local chunk = LoadResourceFile(nt_lib, ('%s/%s.lua'):format(dir, context))
    local shared = LoadResourceFile(nt_lib, ('%s/shared.lua'):format(dir))
    ...
    if chunk then
        ...
        local fn, err = load(chunk, ('@@nt_lib/modules/%s/%s.lua'):format(module, context), 't', env)
...
local NT = setmetatable({
    ...
    onCache = function(key, cb)
        AddEventHandler(('nt_lib:cache:%s'):format(key), cb)
    end
}, {
    __index = call,
    __call = call,
...
_ENV.NT = NT
for i = 1, GetNumResourceMetadata(nt_lib, nt_lib) do
    local name = GetResourceMetadata(nt_lib, nt_lib, i - 1)

    if not rawget(NT, name) then
        local module = loadModule(NT, name)
